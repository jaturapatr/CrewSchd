"""
CrewSchd - Master Roster Exporter (Block-Based View)
Converts JSON rosters into an easy-to-read staff-vs-day time table.
"""
...
def get_roster_explanation(roster_data, employee_data, weather_data):
    ...
    try:
        ...
        return "System optimization successfully balanced team coverage using 4h micro-shifts."
...
def export_perfect_roster(branch="Main Office", team="Cashier"):
    ...
    for rule in weather.get("daily_overrides", []):
        if rule.get("type") == "block_employee_availability":
            ...
            elif d_obj > leave_map[e_name]["max_date"]:
                leave_map[e_name]["max_date"] = d_obj

    team_counts = {}
    for e in employees.get("employees", {}).values():
        if e["team"] not in team_counts: team_counts[e["team"]] = 0
    for eid, e_data in employees.get("employees", {}).items():
        if e_data["name"] not in leave_map: team_counts[e_data["team"]] += 1

    absences = []
    for name, data in leave_map.items():
        absences.append({"name": name, "reason": data["reason"], "return": (data["max_date"] + timedelta(days=1)).strftime('%a %d %b')})
    ...
    <head>
        <style>
            ...
            .roster td {{ border: 1px solid #e1e4e8; padding: 10px; font-size: 11px; text-align: center; }}
            ...
            .absent-item {{ background: #fff5f5; border: 1px solid #feb2b2; color: #c53030; padding: 8px 12px; border-radius: 6px; margin-bottom: 10px; font-size: 13px; }}
        </style>
    </head>
    <body>
        <div class="paper">
            <h1>🛠️ CrewSchd - Weekly 4h-Block Roster ({branch} -> {team})</h1>
            <table class="roster">
                <thead>
                    <tr>
    ...
    for eid in all_emp_ids:
        ...
        for d_str in sorted_dates:
            ...
            if blocks_assigned:
                sorted_blocks = sorted(blocks_assigned)
    ...
    html += f"""
            ...
            <div class="summary-section">
                <div class="strategy-box"><strong>💡 Operations Strategy:</strong><br>{strategy_summary}</div>
                <div class="card"><h3>Available Headcount</h3>
                    <table style="width:100%; font-size: 13px;">
                        {"".join([f"<tr><td>{t}</td><td style='text-align:right'><b>{c} Staff</b></td></tr>" for t, c in sorted(team_counts.items())])}
                    </table>
                </div>
                <div class="card"><h3>Absences</h3>
                    {"".join([f"<div class='absent-item'><b>{a['name']}</b>: {a['reason']}<span style='display:block; font-size:10px; color:#e53e3e;'>Return: {a['return']}</span></div>" for a in absences]) if absences else "None."}
                </div>
            </div>
        </div>
    </body>
    ...
    print(f"✨ UPDATED BLOCK-BASED ROSTER for {branch}/{team}: Created at {output_path}")
